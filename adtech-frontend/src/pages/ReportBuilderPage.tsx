...
import {
  ...
  ReportQueryRequest, AdReportData // <--- CRITICAL FIX: Changed AdReportDto to AdReportData here
} from '../api';
import { debounce } from '../utils/debounce';
import dayjs from 'dayjs';
...
const ReportBuilderPage: React.FC = () => {
  const [reportData, setReportData] = useState<AdReportData[]>([]); // <--- CRITICAL FIX: Changed AdReportDto[] to AdReportData[] here
  ...
  const getReport = useCallback(async (page: number, size: number, sortBy?: string, sortOrder?: 'ASC' | 'DESC') => {
    ...
    try {
      ...
      setReportData(response.content); // This now correctly receives AdReportData[]
  ...
  const getColumns = (): TableProps<AdReportData>['columns'] => { // <--- CRITICAL FIX: Changed AdReportDto to AdReportData here
